# TODO:
# Add spcaed repetition.
# Check all folders for text files for question.
# Run script perminatly to prompt me to do a test at optimal times for memory
...
try:
    f = open(
        os.path.join(qpath, sys.argv[1][0].lower(), f"{sys.argv[1][1]}.txt"))
except:
...
results['Date'] = date
if os.path.exists(
        os.path.join(rpath, sys.argv[1][0].lower(),
                     f'{sys.argv[1][1].lower()}.csv')):
    with open(os.path.join(rpath, sys.argv[1][0].lower(),
                           f'{sys.argv[1][1].lower()}.csv'),
              'a+',
              newline='') as csvfile:
...
def check_files():
    for root, dirs, filess in os.walk(qpath):
        for file in filess:
